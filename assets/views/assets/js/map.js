...
function initMap() {
    ...
    if (!!map_location.lat && !!map_location.lng) {
        ...
        gGeocode.geocode( { 'address': map_location.address }, function(result, status) {
 ...
 function generateMap(location) {
    ...
    var mapMarker = wpAssetUrl + '/assets/img/icon-map-marker.png';

    var infoContent = '<h4>' + location.name + '</h4>' +
